...
| 項目 | 値 |
|---|---|
| 買い注文上限 | min(3,000,000 * 価格, YD残高の10%) |
| 売り注文上限 | min(3,000,000, YDC残高) |
| 手数料 | 1%（固定） |
| サーキットブレーカー | ±30%（価格変動がこの範囲を超えると取引拒否） |
| 日次取引量制限 | `tradedAmountToday` で監視（閾値に達すると拒否） |
...
- YDCには日次制限・注文上限・サーキットブレーカーがありますが、MKCにはありません
